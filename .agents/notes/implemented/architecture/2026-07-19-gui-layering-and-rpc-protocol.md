...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` (`@deepseek-ai/dsh-desktop`) is the Electron application: it reuses the same Web client and webserver over an ephemeral loopback carrier inside Electron; the [desktop decision](../feature/2026-08-16-electron-desktop-tencent-codebuddy.md) owns that deployment, while an IPC carrier remains a possible later substitution.
