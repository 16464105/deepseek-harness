...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop` 在 Electron 内部通过临时回环载体复用同一套 Web client 与 webserver；该部署由[桌面决策](../feature/2026-08-16-electron-desktop-tencent-codebuddy.md)负责，IPC 载体仍可作为后续替换方案。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 通过 HTTP/WebSocket 承载浏览器；零 workspace 依赖（注册表经结构注入到达）；Electron 应用在私有回环上复用它 |
...
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web）+ `@deepseek-ai/dsh-desktop`（apps/desktop） | bin 粗分发、`dsh-client-web` 之上的薄 Vite main，以及围绕 desktop profile 的 Electron 生命周期 | dist 定位、profile 选择和原生窗口等 workspace 与生命周期知识保留在 `apps/` |
