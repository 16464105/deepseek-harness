...
**Tier 2 — upstream extension points, which the fork cannot supply alone.** Eleven upstream source files still carry fork behavior, in four groups:

- `packages/api/session-controller` adds `openDirectory`, `canOpenPath`, and `refresh` to the skill catalog, and declares the model input modalities the model seat reads.
- `packages/client/ui-model-selection` blocks text-only routes while an image rides the next request, which needs a capability-decoration extension point on the single-occupant `conversation.input.model` seat.
