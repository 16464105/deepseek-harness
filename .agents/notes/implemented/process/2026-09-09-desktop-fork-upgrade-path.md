...
**Tier 2 — upstream extension points, which the fork cannot supply alone.** Sixteen upstream files still carry fork behavior, in four groups:

- `packages/api/remotes` and `packages/api/session-controller` mount the MCP and browser-popup Remotes and add `openDirectory`, `canOpenPath`, and `refresh` to the skill catalog.
...
- `packages/llm/llm-pi-ai` and `packages/skill/skill` carry the `prepareRequest` adapter hook and the public `SkillRegistry.refresh()`.
