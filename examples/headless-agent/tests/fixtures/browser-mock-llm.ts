...
class BrowserMockAdapter extends LlmAdapter {
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (toolResult === undefined) {
      ...
      yield { type: 'tool-call-delta', index: 0, id: CallId('browser-nav-call'), name: 'browser_navigate', argumentsDelta: args }
      yield { type: 'block-end', index: 0, block: { type: 'tool-call', id: CallId('browser-nav-call'), name: 'browser_navigate', arguments: args } }
    ...
    if (toolText.includes('Navigated to')) {
      ...
      yield { type: 'block-start', index: 0, blockType: 'tool-call' }
      yield { type: 'tool-call-delta', index: 0, id: CallId('browser-snap-call'), name: 'browser_snapshot', argumentsDelta: args }
      yield { type: 'block-end', index: 0, block: { type: 'tool-call', id: CallId('browser-snap-call'), name: 'browser_snapshot', arguments: args } }
      yield { type: 'usage', usage: { inputTokens: 11, outputTokens: 3, cacheReadTokens: 2 } }
      yield { type: 'finish', reason: { kind: 'tool-calls' } }
      return
    }
