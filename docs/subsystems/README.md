...
| [mcp.md](mcp.md) | how the Host reaches external Model Context Protocol servers: `mcp-client`'s per-server transport, tool discovery, reconnect supervisor, and model-facing tool generation, plus `mcp-manager`'s persisted definitions, secret hiding, and child-fiber reconciliation |
