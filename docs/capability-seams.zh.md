...
flowchart LR
  ...
  svc_mcpManager["ctx.mcpManager<br/>Managed MCP server lifecycle"]
  pkg_ui_settings_mcp["ui-settings-mcp"]
  ...
  svc_mcpManager --> pkg_ui_settings_mcp
...
| `ctx.mcpManager` | `seam` | [`mcp-manager`](../packages/mcp/mcp-manager) | [`mcp-manager`](../packages/mcp/mcp-manager) | `ui-settings-mcp` | - | Host 持久化隐藏敏感字段的服务器定义，并为每个启用条目协调一个 mcp-client fiber；Web 设置消费者拥有编辑与生命周期控件。 |
