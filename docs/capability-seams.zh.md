...
| `ctx.mcpManager` | `seam` | [`mcp-manager`](../packages/mcp/mcp-manager) | [`mcp-manager`](../packages/mcp/mcp-manager) | `ui-settings-mcp` | - | Host 持久化脱敏后的服务器定义，并为每个启用的条目协调一个 mcp-client fiber；Web 设置消费者拥有编辑与生命周期控制。 |
