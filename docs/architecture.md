...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all; [`dsh-desktop-app`](../packages/bundle/desktop-app/README.md) layers over base + web-app for the Electron launcher, selecting a private loopback deployment and Tencent CodeBuddy defaults. The Tencent adapter keeps endpoint and protocol fixed while projecting the local CodeBuddy client's desktop-chat cache into `ctx.llm`; its settings namespace refreshes that cache and controls which models the shared chat selector lists.
