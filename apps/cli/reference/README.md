...
The `web`, `headless`, and `desktop` profiles auto-initialize from shipped templates on first use (`web`: base + web-app; `headless`: base + headless; `desktop`: base + web-app + desktop-app). The Electron application owns the desktop launch. Any other missing profile fails loud with a hint to run `dsh plugin --profile <name> add <package>`.
...
All CLI modes treat the invoking directory as the default workspace root, load applicable `AGENTS.md` or `CLAUDE.md` instructions with a 65,536-byte render budget, and use an in-memory SQLite session content index. Every CLI profile boot watches valid edits of both `cordis.patch.yml` layers (profile and home) and reapplies them transactionally; a one-shot surface exits through its bounded shutdown, which disposes the watchers. The Electron desktop launcher deliberately disables these watchers because that packaged runtime does not provide their Node HMR facilities.
