...
Build the Windows x64 zip (contains `DeepSeek Harness.exe`) when every native dependency has a Windows x64 prebuild. NSIS still requires a Windows runner or Wine (`makensis`). The repository's `supportedArchitectures` setting installs those Windows optional packages during the ordinary workspace install:
