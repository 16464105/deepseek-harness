...
- **Windows Node hosts have no mechanism fallback** — outside Electron, the koffi child-process picker is the only native tier, so a COM refusal or dialog crash surfaces the failure. The packaged desktop Host uses Electron's `dialog.showOpenDialog` instead of that child. The browse backend remains the fallback at the composition level.
