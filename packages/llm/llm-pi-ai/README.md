...
The package root exposes the Cordis plugin contract, `PiAiAdapter`, `resolveProfiles()`, and `supportedProtocols()`. `resolveProfiles()` is the validated resolve step reused by fixed-provider adapters. `PiAiAdapter` additionally accepts a `prepareRequest` hook that can contribute per-request headers and transform the provider payload immediately before dispatch; provider construction, replay conversion, and stream conversion remain package-internal.
