...
包根入口导出 Cordis 插件约定、`PiAiAdapter`、`resolveProfiles()` 与 `supportedProtocols()`。`resolveProfiles()` 是供固定提供方适配器复用的、带校验的显式解析步骤。`PiAiAdapter` 还接受 `prepareRequest` hook，用于逐请求添加标头，并在发送前转换提供方 payload；提供方构造、回放转换和流转换仍保留在包内部。
