...
供 app bin（[`dsh`](../../../apps/cli/README.md)、[`dsh-desktop`](../../../apps/desktop/README.md) 与 [`dsh-acp-demo`](../../examples/acp-demo/README.md)）共用的启动粘合层：每个 bin 都是在这些辅助函数之上构建的精简自执行组合，并以自身诊断前缀参数化。这样，Loader 故障行为只由一处负责，不会在已发布产物之间逐渐分化。
...
- **环境裸包解析仍依赖宿主**：未传入 `bareModuleBaseUrl` 的调用方依赖 Loader 私有适配器或原生 import 的可见范围。封闭运行时必须提供该基准，并维护完整的已安装依赖闭包。
