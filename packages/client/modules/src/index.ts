...
export class ClientModuleRegistry extends Service {
  ...
  private locatePkgJson(loaderName: string, baseUrl: string): { path: string; packageName: string } | undefined {
    ...
    // The installation resolver (packaged hosts) turns a bare package name
    // into a file URL inside the archive; prefer it over Node internals so
    // asar-backed profiles resolve their plugin manifests.
    if (!pathLike && this.ctx.loader.resolveImport !== undefined) {
      const resolved = this.ctx.loader.resolveImport(`${expectedPackageName}/package.json`, baseUrl, {})
      if (resolved.startsWith('file:')) {
        // The early return above narrowed `pathLike` to false, which makes
        // `expectedPackageName` definite; TS cannot see through the ternary.
        return { path: fileURLToPath(resolved), packageName: expectedPackageName as string }
      }
    }
