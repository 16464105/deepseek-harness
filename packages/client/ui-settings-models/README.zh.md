...
腾讯卡片采用 DeepSeek 的形态：API 密钥字段——上方带一条指向 `https://tencent.sso.codebuddy.cn/profile/keys` 的提示链接，CodeBuddy Key 在那里生成——加上收起在「自定义设置」折叠区里的适配器模型目录。固定 catalog 由本包持有（其 schema 默认值会物化继承行），因此折叠区无需任何操作即显示随包交付的 29 个模型，并用与 DeepSeek 编辑器相同的 id/名称/容量行编辑；首次编辑会把完整数组物化到用户层，**恢复默认模型**则取消该覆盖。端点保持固定，因此卡片不提供 API 地址或获取控件。
