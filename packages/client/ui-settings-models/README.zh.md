...
模型设置与产品引导插件。同一个 client Cordis 插件会注册 Models 页面和两个有序的首次使用弹窗：版本化内测声明，以及按条件显示的首选提供方凭据步骤。两个步骤共用同一套弹窗组件，并继续由 `settings.onboarding` 排序。Models 平面把三个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）与 `credentials.describe`（不含值的 configured/source/writable 徽标）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。
...
声明完成后，提供方步骤会从同一个 Models 联接快照得出首次运行就绪状态。只要用户已经能触达**任何**一个提供方，它就直接完成而不渲染——已注册且其具名凭据引用已存储的路由（包括来自启动环境且只读的凭据），或 profile 根本不指名引用、因而走原生认证的路由。没有任何就绪提供方时，协调器会按部署顺序选择第一个已组合的受支持目标：先选腾讯 CodeBuddy，再选 DeepSeek 官方。目标适配器已挂载且活跃、引用可写但尚未配置时，`ProviderEditor` 会以仅凭据模式和对应提供方文案渲染在共用引导弹窗中；`credentials.set` 仍是唯一的 secret 写入，且不会改变提供方设置。「稍后配置」只完成协调器当前这一轮。没有受支持适配器、路由不活跃、联接失败、部署只读或设置／凭据能力不可用时，该步骤不渲染并直接完成；Models 页仍是诊断界面。
...
腾讯卡片采用 DeepSeek 的形态：API 密钥字段——上方带一条指向 `https://tencent.sso.codebuddy.cn/profile/keys` 的提示链接，CodeBuddy Key 在那里生成——加上收起在「自定义设置」折叠区里的适配器模型目录。固定 catalog 由本包持有（其 schema 默认值会物化继承行），因此折叠区无需任何操作即显示随包交付的模型，并用与 DeepSeek 编辑器相同的 id/名称/容量行编辑；首次编辑会把完整数组物化到用户层，**恢复默认模型**则取消该覆盖。在腾讯卡片上新增的行会预填图片输入与完整的推理等级阶梯（`low`…`max`），让手工添加的模型也能真正用上这两项能力；保存的行会保留目录承载的每一项能力字段，绝不会保存被剥离的拷贝。端点保持固定，因此卡片不提供 API 地址或获取控件。
...
- **卡片上可编辑的只有 API 密钥与精选提供方控件**：手写编辑器用 schema 通用的字段覆盖面换来了设计稿上的布局（[Agent Note](../../../.agents/notes/implemented/architecture/2026-07-30-web-config-plane.md)）。DeepSeek 与 pi-ai 公开 `baseURL` 和模型的 `id`/`name`/`contextWindow`/`maxTokens`；手工声明的 pi-ai 路由还公开 `displayName` 与 `api`，腾讯则公开其固定模型目录。重试策略、超时、DeepSeek 模型说明及其他进阶字段仍留在 `settings.yaml` 中；编辑器未展示的现有模型字段会予以保留。不带这些约定字段的 profile schema 只渲染该提示，三套精选布局以 `llm-deepseek`/`llm-pi-ai`/`llm-tencent-codebuddy` 这三个 namespace 的名字为键。
