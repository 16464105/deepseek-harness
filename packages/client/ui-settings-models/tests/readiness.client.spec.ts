...
/** The onboarding target these cases exercise: the plugin's own default. */
const PREFERRED = 'deepseek-official'
...
describe('onboardingReadiness', () => {
  it('waits for the first join and skips onboarding when the adapter directory entry is absent', () => {
    expect(onboardingReadiness(state({ status: 'idle', rows: [] }), PREFERRED)).toEqual({ kind: 'loading' })
    expect(onboardingReadiness(state({ status: 'loading', rows: [] }), PREFERRED)).toEqual({ kind: 'loading' })
    expect(onboardingReadiness(state({ rows: [] }), PREFERRED)).toEqual({ kind: 'adapter-absent' })
    ...
    }), PREFERRED)).toEqual({ kind: 'adapter-absent' })
  })

  it('reports a missing writable effective credential', () => {
    expect(onboardingReadiness(state(), PREFERRED)).toMatchObject({ kind: 'credential-missing' })
  })

  it('ends onboarding once any other registered provider can serve requests', () => {
    expect(onboardingReadiness(state({ rows: [row(), otherRow()] }), PREFERRED)).toEqual({ kind: 'provider-ready' })
    // A provider the user cannot reach yet leaves the prompt in place.
    expect(onboardingReadiness(state({
      rows: [row(), otherRow({ credential: missingCredential })],
    }), PREFERRED)).toMatchObject({ kind: 'credential-missing' })
  })
  ...
  it('accepts file and process-environment credentials without prompting', () => {
    ...
    }), PREFERRED)).toEqual({ kind: 'provider-ready' })
    expect(onboardingReadiness(state({
  ...
  it('turns missing capabilities into diagnostics that never block the product', () => {
    expect(onboardingReadiness(state({ status: 'error', error: 'settings down' }), PREFERRED)).toEqual({
      kind: 'unavailable',
      reason: 'load-failed',
    })
    expect(onboardingReadiness(state({
      rows: [row({ entry: { ...row().entry, active: false } })],
    }), PREFERRED)).toEqual({ kind: 'unavailable', reason: 'provider-inactive' })
    expect(onboardingReadiness(state({
    ...
    }), PREFERRED)).toEqual({ kind: 'unavailable', reason: 'credentials-unavailable' })
    expect(onboardingReadiness(state({
      rows: [row({ credential: { configured: false, writable: false } })],
    }), PREFERRED)).toEqual({ kind: 'unavailable', reason: 'credential-read-only' })
    expect(onboardingReadiness(state({ writable: false }), PREFERRED)).toEqual({
      kind: 'unavailable',
