...
目录行还会携带确切模型的可选输入模态。当未发送草稿图片或宿主的 `session.models.requiresImageInput` 状态要求图片输入，而已公布的当前模型明确不含 `image` 时，本插件会保留草稿、注册图片专用的 composer 阻塞块，并打开模型列表。支持图片的行会显示标识；图片仍存在时，明确仅支持文本的行不可选择；能力未知的行仍可选择。这只提供提前反馈：宿主仍会串行处理图片准入与模型选择，并对不兼容的提示词或切换作出权威拒绝。
...
- **模态未知时仍可选择**——省略 `inputModalities` 的适配器不会显示图片标识，也不会被停用；是否接受图片仍由其请求时检查负责。
