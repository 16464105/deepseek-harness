...
Catalog rows also carry the exact model's optional input modalities. When an unsent draft image or the Host's `session.models.requiresImageInput` state requires image input and the advertised current model explicitly excludes `image`, the plugin preserves the draft, raises an image-specific composer block, and opens the model list. Image-capable rows are labeled, explicit text-only rows are disabled while images remain, and rows with unknown capability stay selectable. This is early feedback only: the Host still serializes image admission with model selection and rejects an incompatible prompt or switch authoritatively.
...
- **Unknown modality remains selectable** — an adapter that omits `inputModalities` receives no image label and is not disabled; its request-time guard remains responsible for accepting or rejecting images.
